...
from ARISA_DSML.predict import batch_predict
...
def test_end_to_end_training_pipeline(sample_data, tmp_path, temp_mlflow_home):
    ...
    with patch('ARISA_DSML.train.MODELS_DIR', models_dir), \
         patch('ARISA_DSML.preproc.PROCESSED_DATA_DIR', data_dir):
...
def test_mlflow_logging_verification(sample_data, tmp_path, temp_mlflow_home):
...
def test_batch_prediction_workflow(sample_data, tmp_path):
    ...
    with patch('ARISA_DSML.predict.MODELS_DIR', model_dir), \
         patch('catboost.CatBoostClassifier') as mock_model:
        
        # Mock model predictions
        mock_model.return_value.predict_proba.return_value = \
            np.random.random((len(test_data), 2))
...
def test_error_handling(sample_data, tmp_path):
...
def test_model_registry_operations(temp_mlflow_home):
